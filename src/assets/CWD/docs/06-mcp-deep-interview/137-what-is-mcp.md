...
**MCP (Model Context Protocol)** is an open protocol that standardizes **how an AI agent/LLM connects to external tools, APIs, and enterprise data sources**.

Think of MCP as a **standard communication layer between your AI Worker and enterprise systems**.
...
Enterprise Tool
  ↓
Salesforce / ServiceNow / SharePoint / Database
```
...
| --------------------- | ----------------------------------------- |
| **Worker**            | Decides what business operation is needed |
| **MCP Client**        | Sends the tool request using MCP          |
| **MCP Server**        | Exposes approved enterprise tools         |
| **MCP Tool**          | Performs the actual operation             |
| **Enterprise System** | Salesforce, ServiceNow, SharePoint, etc.  |

### Simple example

Suppose the user asks:

> "Give me a customer briefing for customer C123."

The Coordinator determines the intent:
...
The Sales Delegator invokes a Salesforce Worker.

The Worker doesn't need to directly implement Salesforce API details. Instead, it calls an MCP tool:
...
The Worker calls the appropriate tool with structured parameters:
...
The MCP server validates the request and invokes the underlying Salesforce integration.

### Why use MCP?

Without MCP, every Worker may need its own integration:
...
Standardized tools
   ↓
Salesforce / ServiceNow / SharePoint / SAP
```

This gives you **standardized tool invocation, centralized security, validation, auditing, and easier reuse**.

### MCP vs A2A in your architecture
...
MCP = Agent/Worker ↔ Tool/System communication
```
...
> "MCP, or Model Context Protocol, is a standardized protocol that allows AI agents or Workers to securely discover and invoke external tools and enterprise systems. In our CWD architecture, Workers use MCP to interact with systems such as Salesforce, ServiceNow, and SharePoint, while A2A is used for communication between agents."
